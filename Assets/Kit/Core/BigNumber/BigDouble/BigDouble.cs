...
namespace BigNumber
{
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        {
            return BigNumber.CustomFormat(this);
        }
        ...
        private static class BigNumber
        {
            ...
            public static string CustomFormat(BigDouble value)
            {
                if (value.exponent < 3)
                {
                    string _format = value.ToDouble().ToString("F0");
                    //_format = _format.TrimEnd('0').TrimEnd('.');
                    return _format;
                } 
                else
                    return GetIdleGamesNotation(value);
            }

            private static string GetIdleGamesNotation(BigDouble value)
            {
                long exponent = value.Exponent;
                double multipleOfTen = Math.Pow(10, (exponent % 3));

                string firstPart    = (value.Mantissa * multipleOfTen).ToString("F2");
                string secondPart   = GetSuffixForNumber((int)exponent / 3);
                //string secondPart = GetExponentString(exponent);

                firstPart           = firstPart.TrimEnd('0').TrimEnd('.');

                return $"{firstPart}{secondPart}";
            }

            private static string[] exponentsAbbreviations =
                {
                "k",
                "m",
                "b",
            ...
            private static string GetExponentString(long exponent)
            {
                int abbreviationIndex = (int)(exponent - 3) / 3;
                abbreviationIndex = Math.Min(abbreviationIndex, exponentsAbbreviations.Length);
                return exponentsAbbreviations[abbreviationIndex];
            }
